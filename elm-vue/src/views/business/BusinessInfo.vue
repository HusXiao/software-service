  ...
  <div class="wrapper">
    ...
    <div class="cart">
      ...
      <div class="cart-right">
        ...
        <!-- 做的时候可以先注释掉一个 -->
        <!-- 不够起送费 -->
        <div class="cart-right-item" v-show="totalPrice<business.startPrice"
          style="background-color: #535356;cursor: default;">
          &#165;{{business.startPrice}}起送
        </div>
        <!-- 达到起送费，可以跳转结账 -->
        <div class="cart-right-item" @click="toOder" v-show="totalPrice>=business.startPrice">
          去结算
        </div>
      </div>
    </div>
...
export default {
  ...
  created() {
    ...
    // })).then(responsee => {
    this.$axios.get('BusinessController/getBusinessById/' + this.businessId).then(response => {
    ...
    // 	businessId: this.businessId
    // })).then(responsee => {
    this.$axios.get('FoodController/listFoodByBusinessId/' + this.businessId).then(response => {
  ...
  methods: {
    listCart() {
      ...
      // })).then(responsee => {
      this.$axios.get('CartController/listCart/' + this.user.userId + '/'+this.businessId).then(response => {
    ...
    saveCart(index) {
      ...
      // 	foodId: this.foodArr[index].foodId
      // })).then(responsee => {
      this.$axios.post('CartController/saveCart/'+this.user.userId+'/'+this.businessId+'/'+this.foodArr[index].foodId).then(response => {
    ...
    updateCart(index, num) {
      ...
      // })).then(responsee => {
      this.$axios.put('CartController/updateCart/'+this.user.userId+'/'+this.businessId+'/'+this.foodArr[index].foodId+'/'+(this.foodArr[index].quantity + num)).then(response => {
          if (response.data.result >= 1) {
            // 数据库更新成功
            // 食品数量更新1或-1
    ...
    removeCart(index) {
      ...
      // 	// quantity: this.foodArr[index].quantity+num
      // })).then(responsee => {
      this.$axios.delete('CartController/removeCart/'+this.user.userId+'/'+this.businessId+'/'+this.foodArr[index].foodId).then(response => {
        if (response.data.result == 1) {
          // 数据库删除成功
