...
<script>
	...
	export default {
		...
		created() {
			...
			//根据orderTypeId查询商家信息
			this.$axios.get('/BusinessController/listBusinessByOrderTypeId/1').then(response => {
		...
		methods: {
			listCart() {
				...
				// })).then(response => {
				this.$axios.get('CartController/listCart/' +'00'+ this.user.userId).then(response => {
					let cartArr = response.data.result;
					//遍历所有食品列表
					for (let businessItem in this.businessArr) {
