...
export default {
  ...
  methods: {
    addUserAddress() {
      ...
      //             )).then(response => {
      // 参数？？！！！
      this.$axios.post('DeliveryAddressController/saveDeliveryAddress/'+this.deliveryAddress.contactName+'/'
          +this.deliveryAddress.contactSex+'/'+this.deliveryAddress.contactTel+'/'+this.deliveryAddress.address+'/'+this.user.userId
      ).then(response => {
